...
class ExampleFullArmMovement:
    ...
    def example_send_joint_angles(self):
        ...
        # My test waypoints
        retractWaypoint = AngularWaypoint()
        retractWaypoint.angles = [0, 340, 180, 214, 0, 310, 90]
        retractWaypoint.duration = 0
        intermediateWaypoint = AngularWaypoint()
        intermediateWaypoint.angles = [355.15, 359.17, 179.76, 250.57, 1.44, 304.2, 82.3]
        intermediateWaypoint.duration = 0
        homeWaypoint = AngularWaypoint()
        homeWaypoint.angles = [0, 15.0, 180, 230, 360, 55, 90]
        homeWaypoint.duration = 0
        custom_waypoint_list = [retractWaypoint, intermediateWaypoint, homeWaypoint]
        ...
        waypoint.oneof_type_of_waypoint.angular_waypoint.append(custom_waypoint_list[0])
        trajectory.duration = 0
        trajectory.use_optimal_blending = False
        ...
        waypoint_num = len(custom_waypoint_list)
        waypoint_idx = 0
        while (waypoint_idx < waypoint_num) :
            try:
            ...
            if (error_number >= 1 and angular_duration != MAX_ANGULAR_DURATION):
                angular_duration += 1
                trajectory.waypoints[waypoint_idx].oneof_type_of_waypoint.angular_waypoint[0].duration = \
                    angular_duration
                ...
                waypoint_idx += 1
                angular_duration = 0
                if (waypoint_idx >= waypoint_num):
                    rospy.loginfo("All waypoints validated")
                    print(trajectory)
                    break
                waypoint = Waypoint()
                waypoint.oneof_type_of_waypoint.angular_waypoint.append(custom_waypoint_list[waypoint_idx])
                trajectory.waypoints.append(waypoint)
                trajectory.waypoints[waypoint_idx].oneof_type_of_waypoint.angular_waypoint[0].duration = \
                    angular_duration
        ...
        # Send the angles
        rospy.loginfo("Enter to send custom waypoints...")
        input()
        try:
            self.execute_action(req)
        except rospy.ServiceException:
    ...
    def create_joint_velocity_msg(self, joint_vel):
        base_joint_speeds = Base_JointSpeeds()
        assert(len(joint_vel) == 7), "Joint velocity command must be 7 dofs"
        for idx in range(len(joint_vel)):
            joint_speed = JointSpeed()
            joint_speed.joint_identifier = idx
            joint_speed.value = joint_vel[idx]
            base_joint_speeds.joint_speeds.append(joint_speed)
        
        return base_joint_speeds

    def send_velocity_command(self):
        # Note: the robot's high level commands function at a rate of 40Hz. This will be improved in a future release.
        
        duration = 1.0
        # The duration field doesn't have any effect
        # base_joint_speeds.duration = 1
        zero_vel = [0, 0, 0, 0, 0, 0, 0]

        # The maximum joint velocity is 1.4 rad/sec or 80 degrees/sec
        joint_vel = [-2, 0, 0, 0, 0, 0, 0] # radians/sec

        vel_cmd = self.create_joint_velocity_msg(joint_vel)

        self.joint_velocity_pub.publish(vel_cmd) 
        rospy.sleep(duration)
        # vel_cmd = self.create_joint_velocity_msg(zero_vel)
        # self.joint_velocity_pub.publish(vel_cmd)
    
    def replay_velocity_from_demonstration(self):
        demo_traj = np.loadtxt(open(self.data_addr+"test.csv"), delimiter=",")
        zero_vel = [0, 0, 0, 0, 0, 0, 0]

        sample_rate = 100
        rate = rospy.Rate(sample_rate)
        for i in range(demo_traj.shape[0]):
            joint_vel = demo_traj[i,7:14].tolist()
            vel_cmd = self.create_joint_velocity_msg(joint_vel)
            self.joint_velocity_pub.publish(vel_cmd)
            rate.sleep()
    ...
    def main(self):
        ...
        if success:
            ...
            self.replay_velocity_from_demonstration()
